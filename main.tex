...
\indent Imprenta Lux S.A. es una empresa del sector gráfico, especializada en la producción de diversos materiales impresos, tales como libros, revistas, almanaques, volantes y afiches.\\
\indent La empresa tiene su sede principal en Hipólito Yrigoyen 2463 (Santa Fe, Argentina), aunque también cuenta con una sede secundaria dedicada exclusivamente a tareas de producción. Sin embargo, en las dos instalaciones se llevan a cabo procesos productivos, y, a su vez, en ambas existen sectores destinados al depósito de insumos.\\
\indent La dispersión geográfica suele representar una dificultad significativa para la correcta planificación de los proyectos por parte del gerente general de la empresa, ya que no es poco común que no se cuente con datos actualizados provenientes de relevamientos sobre la disponibilidad de insumos en los depósitos. Esta situación suele generar demoras en las respuestas que deben darse conforme a las necesidades del cliente.\\
\indent El documento que se presenta a continuación describe el proceso llevado a cabo para diseñar e implementar un sistema de información destinado a resolver la situación de interés que ya fue adelantada y que será detallada en los apartados siguientes.\\
\indent La redacción del presente informe abarca el período durante el cual se desarrolló el proyecto, finalizando con la verificación del cumplimiento de los objetivos establecidos en el Plan de Proyecto presentado a la cátedra el 10/06/2024. Por lo tanto, el intervalo temporal relevante comprende desde el 01/07/2024 hasta el 25/04/2025.
...
\indent Respecto a la gestión de riesgos del proyecto, en el \hyperlink{apendice_d}{Apéndice D} se muestra el \textit{Plan de gestión de riesgos}, el cual había ya sido presentado a la cátedra de PFC en la instancia correspondiente al Plan de Proyecto.
...
\indent Dado el tamaño y las necesidades operativas de la empresa, se optó por una arquitectura de tipo 2-tier (cliente-servidor), en la cual tanto la aplicación de escritorio como la aplicación móvil contienen la lógica de presentación y de negocio, y acceden directamente a la base de datos ubicada en un servidor remoto contratado. La decisión de elegir este tipo de arquitectura se fundamenta en que se prevé que el sistema sea utilizado de forma simultánea por un número reducido de usuarios (no más de cinco en promedio), lo cual hace innecesaria la complejidad adicional de una capa intermedia.\\
...
En el \hyperlink{apendice_e}{Apéndice E} (figura \ref{diagrama_de_clases}) se muestra el diagrama de clases que define la estructura del sistema.

\subsection{Estructura de la base de datos}
Considerando la experiencia previa del equipo en el uso de bases de datos relacionales, se optó por su implementación en este caso. En el \hyperlink{apendice_e}{Apéndice E} (figura \ref{diagrama_ER}) se muestra el diagrama de entidad-relación (notación de Chen) que define la estructura de la base de datos del sistema.
...
Si bien en el Plan de Proyecto, en lo relativo al control de calidad, únicamente se había indicado que se realizarían pruebas \begin{math}\alpha\end{math} (\textit{Alpha}), lo realizado en la práctica estrictamente no puede considerarse como tal en este caso, ya que las pruebas se asemejaron más a una inspección superficial por parte del usuario, y con intenciones de captar aspectos a tener en cuenta para las siguientes etapas por parte del equipo de desarrollo.\\
...
A continuación, se describen las discrepancias surgidas durante la ejecución del proyecto en las distintas etapas previstas en el Plan de Proyecto:\\
...
\end{itemize}
\indent En tanto hubieron diferencias entre los tiempos planificados inicialmente y los reales, también hay variaciones entre la diagramación presentada en la etapa de Plan de Proyecto y la observada en la práctica. En las figuras \ref{diagrama de gantt 1} y \ref{diagrama de gantt 2} se muestran respectivamente los cronogramas correspondientes a estos dos casos.
...
\begin{figure}[H]
	...
	{%
		\setlength{\fboxsep}{0pt}%
		\setlength{\fboxrule}{0.5pt}%
		\fbox{\includegraphics[scale = 0.7]{Diagrama_Gantt_1.png}}%
	}%
	\caption{Diagrama de Gantt del proyecto presentado en la etapa de Plan de Proyecto.}
...
\begin{table*}[h!]
	...
	\begin{tabularx}
		...
		\multicolumn{2}{| X |}{Cada depósito de la empresa deberá tener asociada una cuenta de los insumos que se tiene a disposición en cada uno de ellos.\newline
			...
			El encargado de reposición solo podrá modificar los valores aumentándolos o decrementándolos (solo establecer cantidades relativas a la disponibilidad previa en los inventarios), mientras que el encargado de mantenimiento puede establecer una cantidad absoluta (independiente del valor previo).\newline
...
\begin{table*}[h!]
	...
	\begin{tabularx}
		...
		\multicolumn{2}{| X |}{El encargado de producción tanto como los empleados de producción asignados a una tarea pueden modificar el estado de la tarea.
			...
			\end{itemize}
			Si se modifica el estado de una tarea, se debe notificar al encargado de producción.\newline
...
\chapter*{Apéndice C\\Objetivos del proyecto}
...
\chapter*{Apéndice D\\Plan de gestión de riesgos}
...
\begin{table*}[h!]
	...
	\addcontentsline{lot}{table}{D.\arabic{counter_tbl_D}. Matriz de Probabilidad e Impacto.}\stepcounter{counter_tbl_C}
...
\begin{enumerate}
	...
	\indent Por otro lado, dado que la estrategia solo es aplicable en iteraciones distintas de la primera, en esta se deberá intentar subestimar la velocidad de modo de contemplar el tiempo de investigación, adquisición de conocimientos fundamentales así como período inicial de acostumbramiento a las tecnologías utilizadas.
	\item \underline{\smash{\Paste{riesgo_2}}:}\\
...
\clearpage
\phantomsection
\hypertarget{apendice_e}{}
{
	\centering
	\vspace*{\fill}
	\Huge \textbf{Apéndice E}\\[1em]
	\Huge \textbf{Diagramas de clases y de E-R}\\[1em]
	\vspace*{\fill}
	\par
}
\addcontentsline{toc}{chapter}{E. Diagramas de clases y de E-R}
\markboth{Apéndice E - Diagramas de clases y de E-R}{Apéndice E - Diagramas de clases y de E-R}

\sectionmark{Apéndice E - Diagramas de clases y de E-R}
...
\begin{landscape}
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textheight, angle=90]{Diagrama de Clases.png}
...
\chapter*{Apéndice F\\Errores encontrados por iteración}
\markboth{Apéndice F - Errores encontrados por iteración}{Apéndice F - Errores encontrados por iteración}
\sectionmark{Apéndice F - Errores encontrados por iteración}
\addcontentsline{toc}{chapter}{F. Errores encontrados por iteración}
\hypertarget{apendice_f}{}
